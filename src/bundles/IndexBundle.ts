...
const debug = Debug(`poe-dat:`).extend(path.basename(__filename));
...
export function readIndexBundle(indexBundle: Uint8Array) {
  const reader = new DataView(
    indexBundle.buffer,
    indexBundle.byteOffset,
    indexBundle.byteLength
  );
  let offset = 0;
  ...
  let filesInfo: Uint8Array;
  {
    ...
    const begin = offset;
    offset +=
      filesCount * (SFILE_HASH + SFILE_BUNDLEIDX + SFILE_OFFSET + SFILE_SIZE);
    const end = offset;
  ...
  let dirsInfo: Uint8Array;
  {
    ...
    const begin = offset;
    offset +=
      dirsCount * (SDIR_HASH + SDIR_OFFSET + SDIR_DIRECT_SIZE + SDIR_ALL_SIZE);
    const end = offset;
...
export function getFileInfo(
  ...
  if (structOffset == -1) {
    debug(`Failed to find sequence for path %s`, path);
    throw new Error(`Failed to find sequence for path ${path}`);
  }

  const filesReader = new DataView(
    filesInfo.buffer,
    filesInfo.byteOffset,
    filesInfo.byteLength
  );
  const bundleIdx = filesReader.getInt32(structOffset + SFILE_HASH, true);
  const offsetInBundle = filesReader.getInt32(
    structOffset + SFILE_HASH + SFILE_BUNDLEIDX,
    true
  );
  const fileSize = filesReader.getInt32(
    structOffset + SFILE_HASH + SFILE_BUNDLEIDX + SFILE_OFFSET,
    true
  );

  let bundleName: string;
  {
    const bundlesReader = new DataView(
      bundlesInfo.buffer,
      bundlesInfo.byteOffset,
      bundlesInfo.byteLength
    );
    let offset = 0;
    ...
    bundleName = new TextDecoder("utf-8").decode(
      bundlesInfo.subarray(offset, offset + nameLen)
    );
  }
